...
Данный проект распределённо вычисляет арифметические выражения. Сервер, когда ему присылают выражение, разбивает его на задачи, которые отправляются агентам на вычисление. Например, выражение 1+2-3*4/5 будет разбито на задачи: 3 * 4 = 12, 12 / 5 = 2.4, 1 + 2 = 3, 3 - 2.4 = 0.6. Схема:
```mermaid
graph TD;
    1+2-3*4/5-->3*4;
    3*4-->12/5;
...
6. Запустите веб-сервис, введя следующие команды в разных терминалах Visual Studio Code:
- В первом терминале:
```
